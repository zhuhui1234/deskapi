...
//	function __construct(){}
    ...
    protected function mysqlQuery($sql = '', $type = '', $db = '')
    {
        ...
        if (!preg_match("/^(\s*)exec|set|insert|select/i", $sql)) {
            return "SQL ERROR!";
        }
    ...
    protected function mysqlQueryTrue($sql = '', $type = '', $db = '')
    {
    ...
    {
        if ($del) {
            $r_get = $this->redis()->del($key);
            $rs[0] = false;
        } else {
            $rs[1] = $this->redis()->get($key);
            //$rs[0] = json_decode ( $rs[1] , true );
            if ($rs[1] != "") {
                $rs[0] = true;//缓存开
                //$rs[0] = false;//缓存关闭
            } else {
